		...
		if (isset($_GET['id'])) {
			...
			if (!$row) {
				...
				$userpage->replacea([
					...
					"EDITBUTTON" => ((strcmp($_SESSION['current_user'], $row['username']) == 0) ? "<button @click=\"EditAlbum(" . $row['id'] . ")\">Edit Release</button>" : "<button class=\"buy\" @click=\"BuyAlbum(" . $row['id'] . ")\">Buy This Album</button>")
				]);
