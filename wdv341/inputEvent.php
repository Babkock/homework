...
<?php
		if (!isset($_POST['event_id']) || !isset($_POST['event_name']) || !isset($_POST['event_presenter']) || !isset($_POST['event_date']) || !isset($_POST['event_time']) || !isset($_POST['event_description'])) {
			echo "<p><b class=\"error\">One or more fields are empty.</b></p>";
			?>
		</main>
		<footer>
			<p><a href="eventForm.html" title="Previous assignment" alt="Previous assignment">Unit 11: Events Input Form</a></p>
			...
			echo "<p><b class=\"error\">Event date and/or event time fields are malformed.</b></p>";
			?>
		</main>
		<footer>
			<p><a href="eventForm.html" title="Previous assignment" alt="Previous assignment">Unit 11: Events Input Form</a></p>
		...
		<main id="events-form">
			<form action="inputEvent" method="post" enctype="multipart/form-data">
				<center><p class="success">The event was submitted successfully!</p></center>
				<h2>Editing Event "<?php echo $name; ?>"</h2>
				<div class="formFlex">
					<label for="event_id">Event ID:</label>
					<input type="number" name="event_id" value="<?php echo $id; ?>" title="ID of the event. Should be auto-generated" alt="ID of the event. Should be auto-generated" />
				</div>
				<div class="formFlex">
					<label for="event_name">Event Name:</label>
					<input type="text" name="event_name" value="<?php echo $name; ?>" title="Name of the event" alt="Name of the event" placeholder="The Big Big Event" />
				</div>
				<div class="formFlex">
					<label for="event_presenter">Event Presenter:</label>
					<input type="text" name="event_presenter" value="<?php echo $presenter; ?>" title="Who is presenting this event?" alt="Who is presenting this event?" placeholder="Mr. Man" />
				</div>
				<div class="formFlex">
					<label for="event_date">Event Date (<i>in the format YYYY-MM-DD</i>):</label>
					<input type="text" name="event_date" value="<?php echo $tdate; ?>" maxlength="10" title="When does this event take place?" alt="When does this event take place?" placeholder="YYYY-MM-DD" />
				</div>
				<div class="formFlex">
					<label for="event_time">Event Time (<i>in the format HH:MM:SS</i>):</label>
					<input type="text" name="event_time" value="<?php echo $ttime; ?>" maxlength="8" size="9" title="What time does this event start?" alt="What time does this event start?" placeholder="HH:MM:SS" />
				</div>
				<center>
					<label for="event_description"><b>Event Description:</b></label>
					<textarea name="event_description" cols="40" rows="3" title="Enter the description for this event" alt="Enter the description for this event" placeholder="Wow we got some good stuff at this one!"><?php echo $descript; ?></textarea>
				</center>
				<div class="buttons">
					<input type="submit" value="Submit" name="submit" /> &bull;
					<input type="reset" value="Reset" />
